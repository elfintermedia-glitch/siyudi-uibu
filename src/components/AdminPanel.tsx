...
import { StudentAcademic, YudisiumRegistration, WisudaRegistration, DocumentUpload, SystemState, AdminUser } from '../types';
...
export default function AdminPanel({ 
  ...
  // States for academic admin user management inside Academic Admin
  ...
  const handleAddAdminUser = (e: React.FormEvent) => {
    ...
    const newUser: AdminUser = {
      ...
      username,
      password,
      role: 'akademik' // Crucial rule constraint: only 'akademik'
    };

    if (onUpdateAdminUsers) {
      onUpdateAdminUsers([...adminsList, newUser]);
      setAdminSuccess(`Berhasil menambahkan akun staf akademik baru: "${name}".`);
      setNewAdminName('');
      setNewAdminUsername('');
      setNewAdminPassword('');
    ...
    const updated = adminsList.map(u => {
      if (u.id === editingAdminUser?.id) {
        return {
          ...
          username,
          password,
          role: 'akademik' as const // Enforce akademik only rule
        };
      }
      return u;
    });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex border-b border-gray-250 bg-white p-1 rounded-lg shadow-sm">
        ...
        >
          <Users className="w-3.5 h-3.5 animate-pulse" /> Kelola Akun Akademik
        </button>
      </div>
      ...
      {/* Tab: Kelola Akun Staff Akademik */}
      {activeTab === 'admin_users' && (() => {
        const adminsList = state.adminUsers || [];
        const academicAdmins = adminsList.filter(u => u.role === 'akademik');
          ...
          <div className="space-y-6 animate-fade-in text-slate-750">
            ...
            <div className="bg-slate-900 text-slate-100 rounded-2xl p-6 border border-slate-800 shadow-md relative overflow-hidden text-left">
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h2 className="text-xl font-extrabold tracking-tight">Manajemen Akses Staff Akademik</h2>
                  <p className="text-xs text-slate-400 mt-1 max-w-2xl leading-relaxed">
                    Menambahkan, memperbarui, dan mengelola akun staf akademik yang memegang kendali sistem verifikasi mahasiswa. Menjaga privasi data mahasiswa secara terstruktur.
                  </p>
                </div>
                <div className="bg-indigo-950/50 border border-indigo-500/30 px-4 py-2.5 rounded-xl text-center self-start md:self-auto min-w-[150px]">
                  <p className="text-[10px] text-indigo-400 font-bold uppercase tracking-wider">Total Staff</p>
                  <p className="text-2xl font-extrabold text-indigo-300">{academicAdmins.length}</p>
                </div>
              </div>
            </div>

            {/* Information Banner */}
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3 text-amber-900 text-left">
              <Info className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
              <div className="space-y-1">
                <h4 className="font-bold text-xs uppercase tracking-wider">Kebijakan Hak Akses Staf Mandiri</h4>
                <p className="text-[11px] leading-relaxed font-semibold">
                  Divisi Akademik hanya diizinkan untuk membuat dan mengelola akun sesama <strong>Admin Akademik</strong>. Anda tidak diperkenankan membuat akun level Keuangan ataupun Super Admin. Hubungi penanggung jawab IT (Superadmin) jika diperlukan repositori akun lintas divisi.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 pb-6 items-start">
              ...
              <div className="md:col-span-5 bg-white p-5 border border-slate-200 rounded-2xl shadow-sm text-left relative h-fit">
                ...
                <h3 className="font-extrabold text-xs uppercase tracking-wider text-slate-800 border-b border-slate-100 pb-3 mb-4">
                  {editingAdminUser ? 'Edit Data Staff' : 'Formulir Staff Baru'}
                </h3>
                ...
                <form onSubmit={editingAdminUser ? handleSaveEditAdmin : handleAddAdminUser} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Contoh: Rian Anggoro, M.Kom"
                      className="w-full text-xs px-3.5 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 font-semibold bg-slate-50/50 text-slate-800"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="contoh: rian_akademik"
                      className="w-full text-xs px-3.5 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 font-semibold bg-slate-50/50 text-slate-800"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full text-xs px-3.5 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 font-semibold bg-slate-50/50 font-mono text-slate-800"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="w-full text-xs px-3.5 py-2.5 border border-slate-200 rounded-xl bg-slate-50 font-semibold select-none text-slate-500 flex items-center justify-between">
                      ...
                      <span className="text-[9px] bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded border border-indigo-100 font-bold font-mono">TERKUNCI / DIVISI MANDIRI</span>
                    </div>
                    <p className="text-[10px] text-slate-450 mt-1 font-medium italic">Sesuai kebijakan keamanan di atas, divisi terkunci hanya untuk Unit Akademik saja.</p>
                  </div>
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    <button
                      ...
                      className={`text-xs text-white py-3 rounded-xl font-bold transition-all shadow-md cursor-pointer flex items-center justify-center gap-2 ${
                        editingAdminUser ? 'w-2/3 bg-amber-600 hover:bg-amber-750 shadow-amber-500/10' : 'w-full bg-indigo-600 hover:bg-indigo-700 shadow-indigo-500/10'
                      }`}
                    >
              ...
              <div className="md:col-span-7 bg-white p-5 border border-slate-200 rounded-2xl shadow-sm text-left h-fit">
                <h3 className="font-extrabold text-xs uppercase tracking-wider text-slate-800 border-b border-slate-100 pb-3 mb-4 flex items-center justify-between">
                  <span>Daftar Staf Akademik Aktif ({academicAdmins.length})</span>
                  <span className="text-[10px] text-slate-450 lowercase">divisi akademik saja</span>
                </h3>
                  ...
                  <div className="overflow-x-auto overflow-y-auto max-h-[260px] font-sans relative">
                    <table className="w-full text-xs">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {academicAdmins.map((user) => {
                          const isSelf = user.username === currentAdminUsername;
                          return (
                            <tr key={user.id} className={`hover:bg-slate-50/50 transition-colors ${isSelf ? 'bg-indigo-55/30 font-bold text-indigo-950' : ''}`}>
                              <td className="py-3 font-semibold text-slate-800 pl-2">
                                <div className="flex items-center gap-1.5">
                                  <span>{user.nama}</span>
                                  {isSelf && (
                                    <span className="text-[9px] bg-indigo-100 text-indigo-700 px-1.5 py-0.2 rounded font-bold uppercase font-mono">Anda</span>
                                  )}
                                </div>
                              </td>
      ...
      {/* 3. CUSTOM MODAL CONFIRMATION: DELETE STAFF ADMIN */}
      {deletingAdminUser && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in text-slate-750">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-sm w-full overflow-hidden transform transition-all animate-scale-in">
            ...
            <div className="p-5 space-y-3 text-left">
              <p className="text-xs text-slate-550 leading-relaxed font-semibold block">
                Apakah Anda yakin ingin menghapus akun staf akademik berikut? Pengguna tersebut tidak akan bisa mengakses portal lagi.
              </p>
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                ...
                <p className="text-[10px] text-slate-550 font-bold">
                  Grup Akses: <span className="text-slate-700 bg-white px-1 py-0.2 border rounded">Admin Akademik</span>
                </p>
              </div>
            </div>
